...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await postSignup({ tenantName, email, password });
      setSuccess(true);
    } catch (err: any) {
  ...
  if (success) {
    return (
      <div className="w-full relative z-10 dark">
        <div className="bg-[#111827] border border-white/5 rounded-[24px] p-8 sm:p-10 shadow-2xl shadow-black/50 text-center">
          ...
          <Button
            variant="outline"
            className="w-full h-12 rounded-xl bg-transparent border-white/10 text-white hover:bg-white/5"
      ...
      <div className="bg-[#111827] border border-white/5 rounded-[24px] p-8 sm:p-10 shadow-2xl shadow-black/50">
        ...
        <div className="flex flex-col space-y-2 text-center mb-8">
          <h1 className="text-[28px] font-bold tracking-tight text-white flex items-center justify-center gap-2">
            Create an account <span className="animate-wave origin-bottom-right"></span>
          </h1>
          <p className="text-sm text-zinc-400 font-medium">
            Start building with RetrievaAI
          </p>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="space-y-3">
            <Button
              ...
              onClick={() => { window.location.href = `${apiBaseUrl}/api/oauth/google/login`; }}
            >
        ...
        <div className="text-center text-sm mt-8">
          <span className="text-zinc-400 font-medium">Already have an account? </span>
